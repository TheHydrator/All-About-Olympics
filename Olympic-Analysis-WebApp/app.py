...
user_menu = st.sidebar.radio('Olympics Analysis',
...
if user_menu == 'Medal Tally':
    ...
    selected_year = st.sidebar.selectbox('Years', years)
...
if user_menu == 'Country-wise Analysis':
    ...
    # Why This Happens: This issue typically arises if the selected country has no medal data for the given period,
    # which results in an empty DataFrame after filtering. By checking if the DataFrame is empty, you prevent Seaborn
    # from attempting an operation on non-existent data.

    if pt.empty:
        st.write(f"No data available for {selected_country} in the selected period.")
    else:
        fig, ax = plt.subplots(figsize=(20, 20))
        sns.heatmap(pt, annot=True, fmt='g', ax=ax)
        st.pyplot(fig)
...
if user_menu == 'Athlete-wise Analysis':
    ...
    name = []
    famous_sports = [
        'Basketball', 'Judo', 'Football', 'Tug-Of-War', 'Athletics', 'Swimming', 'Badminton', 'Sailing', 'Gymnastics', 'Art Competitions',
        ...
        'Rhythmic Gymnastics', 'Rugby Sevens', 'Beach Volleyball', 'Triathlon', 'Polo', 'Ice Hockey'
    ]
